      ...
      <div
        ...
        className={classNames(`absolute z-20 float-left m-0 hidden 
                ...
                mt-1 bg-[#212529] p-3  
                 ...
                 flex-wrap ${grid ? 'w-[200%] sm:w-[400px] md:w-[600px] [&.active]:flex min-w-[320px]' : 'w-max [&.active]:block '}`, {active}, dropContentClassName)}
...
export const DropItem = ({id, name, value, check, onChangeEvent}: DropItemsProps) => {
  ...
  return (
    <div className='flex items-center w-[48%] sm:w-[32%] md:w-[24%]'>
      <input type="checkbox" id={`${id}`} onChange={handleOnChange} className='checkbox peer hidden' checked={check}
             value={value}/>
...
export const DropItemSimple = ({id, name, value, check, fieldName, onChangeEvent}: DropItemsSimpleProps) => {
  ...
  return (
    <div className='flex items-center w-full xs:w-max'>
      <input type="radio" id={`${id}`} onChange={handleOnChange} name={fieldName || "check-radio"}
             className='checkbox peer hidden' checked={check} value={value}/>
